...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        self.y = tf.placeholder('bool', [N, None], name='y')
        self.y2 = tf.placeholder('bool', [N, None], name='y2')
    ...
    def _build_forward(self):
        ...
        with tf.variable_scope("prepro"):
            ...
            if config.share_lstm_weights:
                ...
                h = tf.concat(2, [fw_h, bw_h])  # [N, JX, 2d]
            else:
        ...
        with tf.variable_scope("main"):
            ...
            (fw_g2, bw_g2), _ = bidirectional_dynamic_rnn(d_cell, d_cell, tf.concat(2, [p0, g1, a1i, g1 * a1i]),
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        if config.len_opt:
            ...
            if sum(len(sent) for para in batch.data['x'] for sent in para) == 0:
                ...
                new_JX = sum(len(sent) for para in batch.data['x'] for sent in para)
            JX = min(JX, new_JX)
        ...
        if supervised:
            y = np.zeros([N, JX + (1 if config.pred_negative else 0)], dtype='bool')
            y2 = np.zeros([N, JX + (1 if config.pred_negative else 0)], dtype='bool')
            feed_dict[self.y] = y
